...
public class SimpleHashtable<K, V> implements Iterable<SimpleHashtable.TableEntry<K,V>> {
  ...
  private class IteratorImpl implements Iterator<SimpleHashtable.TableEntry<K,V>> {
    ...
    /**
     * This node is previous to the middle.
     * */
    private TableEntry<K, V> previousNode;
    /**
     * Number of slot where nextNode is.
     * */
    private int slotNextNode;
    /**
     * Number of slot where middleNode is. This doesn't have to be the same as slotNextNode.
     * */
    private int slotMiddleNode;
    /**
    ...
    public void remove() {
      ...
      if(nextMethodCalled) {
        ...
        if(previousNode == null) {
          table[slotMiddleNode] = table[slotMiddleNode].next;
          return;
        }
        
        if(previousNode.next == middleNode) {
          previousNode.next = middleNode.next;
          return;
        }
        
        // else previousNode.next != middleNode
        table[slotMiddleNode] = table[slotMiddleNode].next;
        return;
      }
   ...
   * */
  private boolean keyExist(TableEntry<K, V> pointer, K key, V value){
    if(pointer.key.equals(key)) {
      pointer.value = value;
      return true;
    }
